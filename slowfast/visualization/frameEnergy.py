...
TEST = [
    '', '001', '002', '003', '004', '015', '016', '017', '018', '019', '020',
    '033', '034', '035', '036', '037', '038', '053', '054', '059', '060',
    '061', '062', '071', '072', '073', '074', '084', '085', '086', '087',
    '096', '097', '098', '099', '100', '101', '116', '117', '118', '119',
...
def max_energy_video(heatmap_num, heatmap_folder, model_name):
    ...
    max_frame_name = "input" + str(heatmap_num) + "pathway1frame" + str(
        max_energy_index) + ".jpg"
    print(max_frame_name)
    input_path = video_folder + max_frame_name

    output_folder = "/research/cwloka/data/action_attn/ucf_high_singleframe_" + model_name + "/" + str(
        heatmap_num) + "/"
...
def min_energy_video(heatmap_num, heatmap_folder, model_name):
    ...
    min_frame_name = "input" + str(heatmap_num) + "pathway1frame" + str(
        min_energy_index) + ".jpg"
    print(min_frame_name)
    input_path = video_folder + min_frame_name

    output_folder = "/research/cwloka/data/action_attn/ucf_low_singleframe_" + model_name + "/" + str(
        heatmap_num) + "/"
...
def rand_energy_video(heatmap_num, heatmap_folder, model_name, j):
    ...
    random_frame_name = "input" + str(heatmap_num) + "pathway1frame" + str(
        random_energy_index) + ".jpg"
    input_path = video_folder + random_frame_name
    print(input_path)

    output_folder = "/research/cwloka/data/action_attn/ucf_rand_" + str(
        j) + "_singleframe_" + model_name + "/" + str(heatmap_num) + "/"
...
def get_boring_videos(loop,
    ...
    if loop:
        for model in ["i3d", "slow", "fast"]:
            if model == 'i3d':
                heatmap_folder = '/research/cwloka/data/action_attn/output_i3d/heatmaps/'
            elif model == 'slow' or model == 'fast':
                heatmap_folder = '/research/cwloka/data/action_attn/output_slowfast/heatmaps/'
            else:
                print(
                    "model name incorrect, please input 'i3d', 'slow', or 'fast'"
                )
                raise ValueError
...
def loop_video(heatmap_num,
    ...
    if rand_iter is None:
        output_folder = "/research/cwloka/data/action_attn/ucf_" + frame_type + "_loop_" + model_name + "/" + str(
            heatmap_num) + "/"
    else:
        output_folder = "/research/cwloka/data/action_attn/ucf_" + frame_type + "_" + str(
            rand_iter) + "_loop_" + model_name + "/" + str(heatmap_num) + "/"
    ...
    for i in range(len(frames)):
        if model_name == 'fast':
            frame_name = "input" + str(heatmap_num) + "pathway1frame" + str(
                frames[i]) + ".jpg"
        else:
            frame_name = "input" + str(heatmap_num) + "pathway0frame" + str(
                frames[i]) + ".jpg"
