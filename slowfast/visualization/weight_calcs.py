...
def get_model_weights(inputs=None, grads=None, activations=None, method="grad_cam"):
    ...
    if method == "grad_cam":
        ...
        reshaped_activations = (
            (activations).reshape(activations.shape[-1], -1).transpose(0, 1)
        )
        ...
        reshaped_activations = reshaped_activations - torch.mean(
            reshaped_activations, axis=0
        )
        ...
        print("sum_activations", sum_activations.shape)
        print("grads", grads.shape)

        numer = grads * activations
        denom = sum_activations[:, :, None, None] + eps

        print("numer", numer.shape)
