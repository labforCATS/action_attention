...
def make_csv(fdir = "/research/cwloka/data/action_attn/kinetics_extracted"
    ...
    with open(os.path.join(fdir, kinetics_test_fname),
                "r",
        ) as f:
    ...
    video_id_list = [kinetics_test_dicts[i]['id'] for i in range(len(kinetics_test_dicts))]
    # label_list = [kinetics_test_dicts[i]['template'] for i in range(len(kinetics_test_dicts))]
    # print(video_id_list)
    # print(label_list)
    ...
    with open(path_to_csv, 'w') as f:
        ...
        # for video_id, label in tqdm(zip(video_id_list, label_list)):
        for video_id in tqdm(video_id_list, total=len(video_id_list)):
            ...
            for frame_id in range(1, 1+len(os.listdir(video_dir))):
                f.write("{} {} {} {} {}\n".format(
                    ...
                    video_id, 
                    frame_id, 
                    os.path.join(fdir, video_id, 
                                 f"{video_id}_{frame_id:06d}.jpg"),
                    '\"\"'
                    ))
...
def make_mini():
    ...
    video_id_list = [kinetics_test_dicts_orig[i]['id'] for i in range(len(kinetics_test_dicts_orig))]
    ...
    for d in kinetics_test_dicts_mini:
        ...
        if not os.path.exists(os.path.join(fdir_mini, d['id'])):
            shutil.copytree(os.path.join(fdir, d['id']), os.path.join(fdir_mini, d['id']))
