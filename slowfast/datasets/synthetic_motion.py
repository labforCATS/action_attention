...
class SyntheticMotion(torch.utils.data.Dataset):
    """
    Video loader for the synthetic motion dataset.

    Construct the Synthetic Motion video loader, then sample
    ...
    def __init__(self, cfg, mode, num_retries=1):
        """
        Construct the Synthetic Motion video loader with a given csv file. The format of
        the csv file is:
        ...
        path_to_folder_of_frames_2 label_2
        ...
        path_to_folder_of_frames_N label_N
        ```
        Load Synthetic Motion data (frame paths, labels, etc. ) to a given
        Dataset object.
        ...
        assert mode in [
            ...
            "val",
            "test",
        ], "Split '{}' not supported for Synthetic Motion".format(mode)
        self.mode = mode
        self.cfg = cfg
    ...
    def _construct_loader(self):
        ...
        csv_path = os.path.join(
            self.cfg.DATA.PATH_TO_DATA_DIR,
            "synthetic_motion_{}.json".format(self.mode),
        )
        with pathmgr.open(csv_path, "r") as f:
            file_json = json.load(f)

        self._video_ids = []
        self._labels = []
        self._path_to_videos = []
        # self._path_to_videos is a list of lists. the ith element corresponds
        ...
        for video in file_json:
            ...
            for frame_num in range(video["num_frames"]):
                frame_file_name = f"{video_id}_{frame_num:06d}.png"
                frame_path = os.path.join(video["path"], frame_file_name)
                frame_paths.append(frame_path)

            self._path_to_videos.append(frame_paths)

        assert len(self._path_to_videos) == len(self._video_ids), (
        ...
        logger.info(
            "Synthetic Motion dataloader constructed "
            " (size: {}) from {}".format(len(self._path_to_videos), csv_path)
        )
