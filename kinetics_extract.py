...
def resize_videos(origin='/research/cwloka/data/action_attn/kinetics/kinetics-dataset/k400/test/'):
    for __, __, files in os.walk(origin):
        ...
        for f in files:
            ...
            if height > width:
                os.system("ffmpeg -i {0}  -vf scale='256:-1' {1}{2}".format(origin+f, DESTINATION_FOLDER, video_name))
            else:
                os.system("ffmpeg -i {0}  -vf scale='-1:256' {1}{2}".format(origin+f, DESTINATION_FOLDER, video_name))
            count += 1
...
def extract_frames(origin=KINETICS_FOLDER, destination=DESTINATION_FOLDER, fps=24):
    ...
    for __, __, files in os.walk(origin):
        for f in files:
            ...
            # Extract the frames
            os.system("ffmpeg -i {0}  -vf fps=fps={3} {1}{2}'_%06d.jpg'".format(origin+f, video_folder, video_name, fps))
