...
# indicates which dataset the plots will be created for #
synthetic_target_masks = False
synthetic_bounding_box = False
synthetic_rotated_bounding_box = True
...
if synthetic_target_masks:
    ...
    max_dict = synthetic_target_masks_maximums
elif synthetic_bounding_box:
    base_data_dir = "/research/cwloka/data/action_attn/synthetic_motion_experiments"
    ...
    max_dict = synthetic_bounding_box_maximums
elif synthetic_rotated_bounding_box:
    base_data_dir = "/research/cwloka/data/action_attn/synthetic_motion_experiments"
    ...
    max_dict = synthetic_rotated_bounding_box_maximums
else: 
    raise NotImplementedError("Exactly one dataset must be set to True in the global variables.")
    ...
    for vis_technique in gc_variants: 
        ...
        for arch in architectures:
            ...
            for channel in channels:
                ...
                for metric in metrics:
                    ...
                    for i in range(len(dataframe_list)):
                        ...
                        for class_id in metric_df["label_numeric"].unique():
                            ...
                            # OFFSET MARKERS
                            ax.scatter(x=mean_df['frameindex'].iloc[class_id], y=mean_df[metric].iloc[class_id], color=vivid_experiment_color_dict[experiment_subset_list[i]], marker = label_marker_dict[classlabel], label=classlabel)
                    ...
                    for i in experiment_subset_list:
                        ...
                        y_limits.append(max_dict[config])
                ...
                print("plotted for", arch, channel, vis_technique)
...
def new_grid(
    ...
    for vis_technique in rows:
        ...
        for arch in cols:
            ...
            axs[row, col].set_xlabel(model_cam_name_dict[arch])
            axs[row, col].set_ylabel(model_cam_name_dict[vis_technique])
...
def gen_all_multi_exp_comparisons():
    for subset in exp_comparisons:
        for softmax in softmax_status:
            ...
            print("finished plots for ", subset, softmax)


def gen_all_grids():
    for subset in exp_comparisons:
        for m in metrics:
            for s in softmax_status:
                new_grid(rows = ["eigen_cam", "grad_cam","grad_cam_plusplus"],
                        cols = ["i3d", "i3d_nln", "fast", "slow"], 
                        experiment_subset_list = subset,
                        metric = m,
                        softmax = s
                        )
            print("grid metric plots for ", subset, m)